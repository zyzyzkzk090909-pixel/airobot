...
本应用定位为“小型AI对话流 App”，目标是统一豆包（Doubao Ark）三条能力链路并在移动端提供顺畅体验：
- 文本对话：多轮会话，自动携带最近 10 条上下文，保持连贯
- 图片理解：支持本地图片与网络图片，自动选择 `input_image` 或 `image_url`
- 图片生成：返回 `b64_json`，在气泡内直接显示
同时提供历史会话（标题/摘要/重命名/删除）、模型横向卡片选择、统一浅灰主题与顶部栏随模式切换、失败重试等完善的产品体验。
...
- 主题与 UI：统一浅灰主题，顶部栏随浅色/深色模式变化，登录页圆角输入框，输入栏贴底
...
## 概述
聊天与多模态助手，支持文本对话、图片理解与图片生成。基于 Kotlin + Jetpack Compose 构建，持久化采用 Room，网络接入使用 Retrofit/OkHttp，主题与字体偏好使用 DataStore。模型接入使用字节跳动火山方舟（Doubao Ark）。

## 主要能力
- 文本对话：多轮会话，自动携带最近 10 条上下文，回复更连贯
- 图片理解：支持本地图片与网络图片
  - 本地 `content://` 自动转 Base64 并以 `input_image` 提交
  - 网络 `http(s)` 直接以 `image_url` 提交
- 图片生成：文生图返回 `b64_json`，聊天气泡内直接显示
- 历史会话管理：自动生成标题（≤12 字）、摘要（首条用户问题 + 最近机器人回答）、支持重命名/删除
- 模型选择卡片：横向卡片切换文本/视觉/文生图及自定义 `ep-...` 接入点
- 统一浅灰主题：顶栏随浅色/深色模式切换（浅色白底/深色暗底），登录页圆角输入框，输入栏贴底
- 失败重试：失败消息右侧显示红色感叹号，点击重试最近用户消息；列表尾部显示统一 Typing 指示

## 架构与技术栈
- 持久层：Room（`message` / `conversation` / `config` / `user`），版本升级与迁移；DataStore 存主题与字体大小
- 仓库层：`OfflineMessageRepository` / `OfflineConversationRepository` / `OfflineConfigRepository` / `OfflineUserRepository`
- 视图模型：`MainViewModel` 统一调度文本、视觉、文生图三条能力链路
- 网络层：Retrofit + OkHttp，Doubao Ark 端点：
...
- 视觉对话（支持 `image_url` 与 `input_image`）：`doubao-seed-1-6-vision-250815`
- 文生图（`b64_json`）：`doubao-seedream-4-0-250828`
- 可扩展：将你的视觉/文本 `ep-...` 接入点加入模型列表并使用
...
- 历史会话：在“历史会话”查看卡片（标题 + 日期 + 摘要），支持重命名与删除；点击切换当前会话
- 模型选择：在“配置”页横向卡片切换模型（文本/视觉/文生图或你的 `ep-...`）
...
- 文生图切为 `b64_json` 展示，避免 URL 访问问题
- 登录与顶部栏统一随主题切换；输入框圆角化与贴底；移除“帮助/开源”入口
- 失败重试改为失败消息 + 感叹号点击重试；移除气泡内“转圈行”，保留列表尾 Typing 指示
- 模型选择改为横向卡片，支持添加 `ep-...`
